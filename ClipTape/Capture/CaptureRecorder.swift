...
final class CaptureRecorder: NSObject {
    ...
    func stop() async throws -> URL {
        guard let stream, let outputURL else {
            throw ClipTapeCaptureError.notRecording
        }
...
enum ClipTapeCaptureError: LocalizedError {
    case notRecording
    ...
    var errorDescription: String? {
        ...
        case .finalizationFailed:
            "ClipTape could not create the final MP4."
        case .screenshotEncodingFailed:
            "ClipTape could not create the screenshot."
        }
    }
}
